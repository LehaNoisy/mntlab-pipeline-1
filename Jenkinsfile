...
try { 
    ...
    stage ('Build') {stagename = 'Build'
        tool name: 'gradle4.6', type: 'gradle';tool name: 'java8', type: 'jdk';tool name: 'groovy4', type: 'hudson.plugins.groovy.GroovyInstallation'
        withEnv(["JAVA_HOME=${ tool 'java8' }", "PATH+GRADLE=${tool 'gradle4.6'}/bin"]){sh 'gradle build'}}
    stage('Test') {stagename = 'Test';withEnv(["JAVA_HOME=${ tool 'java8' }", "PATH+GRADLE=${tool 'gradle4.6'}/bin"]){
        parallel ('Unit Tests': {sh 'gradle test' },'Jacoco Tests': {sh 'gradle jacocoTestReport' },
                'Cucumber Tests': {sh 'gradle cucumber' }, )}}
    stage('Trigger job') {stagename = 'Trigger job';build job: 'MNTLAB-achernak-child1-build-job';sh 'rm -rf *.tar.gz'
    ...
    stage ('Push to Nexus'){stagename = 'Push to Nexus';sh 'groovy push.groovy $BUILD_NUMBER'}
    stage('Approval'){stagename = 'Approval';timeout(time: 10, unit: 'SECONDS'){input message: 'Pull and deploy?', ok: 'pull and deploy'}} 
    stage ('Pull from Nexus'){stagename = 'Pull from Nexus';sh 'groovy pull.groovy $BUILD_NUMBER'}
    stage ('Deploy'){stagename = 'Deploy'
        sh 'tar xvf *${BUILD_NUMBER}.tar.gz'
        sh 'java -jar mntlab-ci-pipeline.jar'}
        notifySuccessful()}
  catch (all) {currentBuild.result = 'FAILED';notifyFailed()}}
